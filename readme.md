...
用自己的域名收发邮件、数据完全自持的个人邮箱系统。单个 Cloudflare Worker 承载收信存储与后台，发送层抽象为可插拔 Provider（当前实现 Resend）。数据落在 Durable Object SQLite（附件等 blob 可选存 DO 或 R2，见「两种存储模式」），无需 D1、无需自建服务器。
...
- **双存储模式**：SQLite 始终只存索引与小正文；`.eml`、附件、超阈值 HTML 正文（>256KB）这类 blob 的落盘位置由 `DATA_DURABLE_MODE` 切换——**DO 模式**（默认，无需绑卡）分片存进 DO 的 SQLite，**R2 模式**存进 R2。业务代码经统一的 `BlobStore` 抽象读写，两种模式对上层完全透明。详见下方「两种存储模式」。
- 所有列表接口强制分页（`page` 默认 1，`pageSize` 默认 20、硬上限 100）。

## 两种存储模式

附件、原始 `.eml`、超阈值 HTML 正文这类 blob 存在哪，由 `wrangler.toml` 里的 `DATA_DURABLE_MODE` 一个变量决定。**邮件索引、正文摘要始终在 DO SQLite，两种模式都不丢信。**

| | **DO 模式**（`"0"`，默认） | **R2 模式**（`"1"`） |
|---|---|---|
| 是否绑卡 | **不需要**，开箱即部署 | 需先绑卡开通 R2 |
| blob 落盘 | 分片存进 DO 的 SQLite | 存进 R2 |
| 容量 | Free 每实例约 1GB（账号总计 5GB）；Paid 每实例 10GB | 大（R2 免费额度 10GB 存储） |
| 大附件下载 | 走 DO、单实例串行，会短暂阻塞其它请求 | 不进 DO、不阻塞 |
| 适用场景 | **只想简单收点邮件**：验证码、通知类低量收信，附件少而小 | **要完整体验**：常收发带附件/大附件、需要高留存 |

- **DO 模式的容量与清理**：本地 SQLite 容量有限，长期收信会撑满。「设置」页提供**历史邮件自动清理**（仅 DO 模式显示）：可设「保留天数」和「最大保留条数」，每日自动删除超限的历史邮件（含附件/正文/原始 .eml），任一命中即删，`0` 表示关闭。默认关闭——按需开启。撰写页上传后未发送的孤儿附件也会在 24h 后自动回收。
- **切 R2 模式**：把 `DATA_DURABLE_MODE` 改为 `"1"`，取消 `wrangler.toml` 里 `[[r2_buckets]]` 三行的注释，`npx wrangler r2 bucket create mail-relay` 后再 `deploy`。
...
npm install
npx wrangler login
npx wrangler deploy                            # 部署 Worker（含 DO 与静态资源），DO 模式无需先建 R2 bucket
...
> 想用 R2 模式（大容量、大附件不阻塞）？见上方「两种存储模式」的切换步骤。
...
npm test            # vitest 集成测试（R2 模式）
npm run test:do     # vitest 集成测试（DO SQLite 模式）
npm run test:all    # 两种模式都跑一遍
```
