...
├─ do/
│  ├─ mailbox.ts          MailboxDO：RPC 方法门面（含密码/设置/邮件操作）
│  ├─ schema.ts           建表 SQL + schema_version 增量迁移（v2 forward_rules，v3 config）
...
前端在仓库根 `public/`（原生 HTML/CSS/JS，无构建）：`index.html`、`style.css`、`app.js`、`mail-frame.js`（正文安全渲染）、`ui.js`（toast/confirm/alert/prompt 组件）。撰写为独立页面 + `contenteditable` 富文本；提示/确认统一走 `ui.js`，不用浏览器原生弹窗。
