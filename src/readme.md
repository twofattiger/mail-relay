...
个人域名邮箱系统：单个 Cloudflare Worker（代码模块化）自建收信存储与后台，发送层抽象为可插拔 Provider（当前实现 Resend）。数据自持于 Durable Object SQLite；附件/原始 .eml/大正文等 blob 由 `DATA_DURABLE_MODE` 一个变量在 **DO 模式（默认，无需绑卡）** 与 **R2 模式** 间切换（见 [§2.1 两种存储模式](#21-两种存储模式)），无需 D1。
...
2. [架构与代码结构](#2-架构与代码结构)
3. [环境要求](#3-环境要求)
4. [注册并配置 Resend（可选）](#4-注册并配置-resend可选)
...
- **收信**：对方 SMTP → Cloudflare Email Routing（MX）→ 本 Worker 的 email handler → 原始 .eml 落盘保底（DO 模式存 DO SQLite、R2 模式存 R2）→ Durable Object 解析入库、线程归并、按规则归档。
...
- SQLite 只存索引与小正文；.eml、附件、超阈值 HTML 正文（>256KB）等 blob 经统一的 `BlobStore` 抽象落盘——DO 模式分片存进 DO SQLite，R2 模式存进 R2（见 [§2.1](#21-两种存储模式)）。
...
| 层 | 位置 | 职责 |
|---|---|---|
| ingest | `src/ingest/handler.ts` | SMTP 会话内 precheck（拒收/转发决策）+ 命中即 `message.forward` + raw 经 `workerBlobStore` 落盘保底（DO/R2）+ 通知 DO；不解析、不写库 |
...
│  ├─ upload.ts           POST /api/upload：撰写页附件上传到 pending 区（DO/R2）
...
├─ do/
│  ├─ mailbox.ts          MailboxDO：RPC 方法门面（含密码/设置/邮件操作/通讯录）+ /__blob/* blob 端点 + alarm
│  ├─ schema.ts           建表 SQL + schema_version 增量迁移（v2 forward_rules，v3 config，v4 contacts，v5 blobs/blob_chunks）
│  ├─ config.ts           config 表读写 + 带默认值的整数配置读取
│  ├─ contacts.ts         通讯录读写、地址解析、发送成功自动入库
│  ├─ ingest.ts           解析入库、线程归并、规则归档、precheck 转发匹配（DoCtx 注入 blob）
│  ├─ send.ts             发送编排、配额、outbox、附件落库、alarm 重试、手动重试
│  ├─ maintenance.ts      DO 模式周期维护：pending 孤儿 GC + 历史邮件自动清理 + 统一 purgeMail
│  └─ crypto.ts           Provider 配置 AES-GCM 加解密
├─ storage/               blob 存储抽象（屏蔽 R2 / DO-SQLite 差异）
│  ├─ index.ts            isR2Mode 模式判定（全项目唯一一处）+ workerBlobStore/doBlobStore 工厂
│  ├─ types.ts            BlobStore 接口（put/get/delete）
│  ├─ r2.ts               R2BlobStore：R2 薄包装
│  ├─ sqlite.ts           SqliteBlobStore：768KB 分片存进 blobs/blob_chunks（putSync 强制单事务）
│  └─ do-proxy.ts         DoProxyBlobStore：Worker 经 stub.fetch(/__blob/*) 读写进 DO
├─ providers/
...
└─ shared/
   ...
   ├─ timeout.ts          withTimeout：SMTP 关键路径硬超时护栏
...
**配置存储**：业务配置集中在 DO 的 `config` 表（`config_name`/`config_value`），键含 `admin_password_hash`、`primary_domain`、`login_max_fails`、`login_lock_seconds`、`daily_send_limit`、`body_inline_max`、`max_mail_size`、`do_retention_days`、`do_retention_max_count`。仅 `CONFIG_MASTER_KEY`/`SESSION_SECRET` 两个加密/签名根密钥保留为 secret（存进被它们保护的库没有意义）。存储模式 `DATA_DURABLE_MODE` 是唯一的普通 `[vars]` 变量，在 `wrangler.toml` 里设置。

依赖规则：`do/` 依赖 `providers/`、`mime/`、`shared/`、`storage/`，不依赖 `api/`、`ingest/`；`api/`、`ingest/` 只认 DO 的 RPC 接口类型 + `storage/` 的 `BlobStore` 工厂，不碰 DO 内部实现。

### 2.1 两种存储模式

附件、原始 `.eml`、超阈值 HTML 正文这类 blob 存在哪，由 `wrangler.toml` 里的 `DATA_DURABLE_MODE` 决定。**邮件索引与正文摘要始终在 DO SQLite，两种模式都不丢信。** 业务代码只认 `storage/` 的 `BlobStore` 抽象，模式判定收敛在 `storage/index.ts` 的 `isR2Mode()` 一处。

| | **DO 模式**（`"0"`，默认） | **R2 模式**（`"1"`） |
|---|---|---|
| 是否绑卡 | **不需要**，开箱即部署 | 需先绑卡开通 R2 |
| blob 落盘 | 分片（768KB/片）存进 DO 的 `blobs`/`blob_chunks` 表 | 存进 R2 |
| 容量 | Free 每实例约 1GB（账号总计 5GB）；Paid 每实例 10GB | 大（R2 免费额度 10GB 存储） |
| 大附件下载 | 走 DO、单实例串行，会短暂阻塞其它请求 | 不进 DO、不阻塞 |
| 适用场景 | **只想简单收点邮件**：验证码/通知类低量收信 | **要完整体验**：常收发带附件/大附件、高留存 |

- **Worker ↔ DO 通道**：DO 模式下 Worker 侧的读写经 `DoProxyBlobStore` 用 `stub.fetch("/__blob/*")` 转发进 DO（原生流式，不受 RPC 体积限制，且不改任何现有 RPC 方法签名）；DO 内部直接用 `SqliteBlobStore`。
- **DO 模式的容量与清理**：本地 SQLite 容量有限。「设置」页提供**历史邮件自动清理**（仅 DO 模式显示）：可设「保留天数」和「最大保留条数」，DO 每日通过 `alarm` 自动删除超限历史邮件（含附件/正文/原始 .eml），任一命中即删，`0`=关闭（默认关闭，按需开启）。撰写页上传后未发送的孤儿 `pending/` 附件也会在 24h 后自动回收。发送中（outbox=queued）的邮件不会被清理。
- **上传上限**：R2 模式单文件 25MB；DO 模式收紧到 10MB（贴合 Free 档 1GB/实例）。
- ⚠️ **模式一经选定不要更改**：`DATA_DURABLE_MODE` 改值后旧数据不会自动搬迁，附件/正文/raw 会读不到（邮件索引与摘要仍在，不丢信）。**请在部署前决定模式。**

依赖规则补充：`storage/do-proxy.ts` 仅按类型引用 `MailboxDO`，不产生运行时循环依赖。
...
- Cloudflare 账号（Workers + Durable Objects；免费档即可）。**默认 DO 模式无需 R2、无需绑卡**；仅在切 R2 模式时才需开通 R2（见 [§2.1](#21-两种存储模式)）。
- Resend 账号（仅在需要发信时注册，可选）。
- 安装依赖：
...
> **💡 提示**：如果你只打算用本系统来**接收**和**转发**邮件，不打算对外发送新邮件或回复邮件，那么可以完全跳过本节以及后续的发信通道配置，无需注册 Resend 账号。
...
3. **（仅 R2 模式需要）创建 R2 bucket**——默认 DO 模式请**跳过本步**。切 R2 模式时，先把 `wrangler.toml` 的 `DATA_DURABLE_MODE` 改为 `"1"`、取消 `[[r2_buckets]]` 三行注释，再创建桶（名字与 `bucket_name` 一致，默认 `mail-relay`）：
...
- `[vars] DATA_DURABLE_MODE`：存储模式开关，默认 `"0"`（DO 模式）。
- `[[r2_buckets]]` 绑定名 `MAIL_R2`：**默认注释掉**（DO 模式不绑 R2，避免 `wrangler deploy` 校验未开通的 R2 而失败）；R2 模式才取消注释。
- `[[durable_objects.bindings]]` 绑定名 `MAILBOX` → 类 `MailboxDO`。
- `[[migrations]]` 声明 `MailboxDO` 为 SQLite-backed Durable Object。
- 除 `DATA_DURABLE_MODE` 外无其它 `[vars]`：每日发送上限、正文外置阈值、登录锁定阈值/时长、DO 历史清理阈值等业务参数都在 DO `config` 表，于后台「设置」页调整。
...
- 想重置本地状态（含已设密码、邮件、附件），删除 `.wrangler/state/v3/do`（DO 模式的 blob 也在其中）与 `.wrangler/state/v3/r2`（仅 R2 模式有）后重启 `wrangler dev` 即可，下次访问会重新走首次引导。
...
npm test              # vitest run（R2 模式：DATA_DURABLE_MODE=1，miniflare 本地模拟 R2）
npm run test:do       # vitest run --config vitest.do.config.ts（DO SQLite 模式：不绑 R2）
npm run test:all      # 两种模式各跑一遍
```

**同一套用例在两种模式下都跑**：测试里 blob 读写统一走 `test/helpers.ts` 的 `testBlob(env, stub)`（R2 模式走全局 R2；DO 模式写入该测试自己的 DO），从而走与生产完全相同的代码路径。两套 vitest 配置分别注入 `DATA_DURABLE_MODE=1/0`。

测试在本地 workerd 里模拟 Durable Object / R2 / SQLite，覆盖：

- MIME 解析：地址规范化、snippet、References 抽取。
- 收信：precheck 放行/拒收、幂等去重（重复 message-id）、线程归并（In-Reply-To）、正文超阈值外置 blob、解析失败兜底、规则自动归档。
...
- 邮件操作：标记已读/未读、移动文件夹、彻底删除并清理 blob（含表行）。
- **blob 存储（`test/blob-store.test.ts`）**：`SqliteBlobStore` 分片往返（空 blob、恰好 1 片、跨多片逐字节相等、覆盖写不残留、分片缺失流以 error 中断、contentType=null 往返）。
- **blob 端到端（`test/blob-e2e.test.ts`）**：收带附件 .eml → 列表可见 → 附件与 raw 字节一致 → purge 清空 blobs/blob_chunks。
- **SMTP 护栏（`test/timeout.test.ts`）**：`withTimeout` 对永不 resolve 的 put 在阈值内抛错（复现原「附件接收致 SMTP 无响应」bug 的失败模式），不挂起。
...
Worker 部署后默认域名是 `https://mail-relay.<你的子域>.workers.dev`。**`*.workers.dev` 在中国大陆常被阻断，大概率打不开后台**，所以国内使用请先给 Worker 绑一个自定义域名，之后所有「打开后台」的操作都用这个自定义域访问。

1. Cloudflare 控制台 → **Workers & Pages** → 点开本 Worker（`mail-relay`）。
2. **Settings（设置）→ Domains & Routes（域和路由）** → **Add（添加）→ Custom Domain（自定义域）**。
3. 填一个已托管在 Cloudflare 的域名或子域，**建议用子域**（如 `mail.yourdomain.com`），别直接用收信主域，免得和收信/发信配置搅在一起 → 保存。
4. Cloudflare 自动写入解析并签发证书，通常几分钟内生效。之后用 `https://mail.yourdomain.com` 打开后台即可。

> 这个自定义域只负责「HTTP 访问后台」，和收信用的 MX（Email Routing）、发信用的 Resend 子域互不冲突，三者各管一段、可以并存。收信仍然是发往 `任意前缀@你的收信域`。

### 接通步骤

Worker 部署且 secret 注入后：

1. **设置管理密码**：浏览器打开后台（国内用上面绑定的自定义域，如 `https://mail.yourdomain.com`；否则用 `*.workers.dev` 域名）→ 首次访问进入「初始化」页 → 设置后台管理密码（至少 6 位，PBKDF2 哈希入库）。
...
4. **配置发信通道（可选）**：左侧「发送通道」→ **新增通道** → 选择 Resend → 粘贴第 4 步创建的 Resend API key（可选填发件人显示名）→ 保存。如果你不需要发信功能，请忽略此步。
5. **测试连接（可选）**：在通道列表点「测试」，会调用 Resend 校验 key；成功后点「激活」。同一时刻只有一个激活通道，发信即走它。
...
- **存储额度**：DO SQLite 免费档约 1 GB/对象；写满后 INSERT/UPDATE 报 `SQLITE_FULL`，读与删除仍可用。
  - **DO 模式**（默认）：附件/原始 .eml/大正文都分片存在这 1 GB 里，容量最吃紧——请在「设置」页开启历史邮件自动清理（保留天数/最大条数），并优先只收验证码、通知类邮件。重附件场景建议切 R2 模式。
  - **R2 模式**：大对象都在 R2（免费 10 GB），DO SQLite 只存索引与小正文，正文外置规则兜底。
- **切换存储模式后附件打不开**：`DATA_DURABLE_MODE` 改值后旧数据不会自动搬迁，附件/正文/raw 的 `blob.get()` 会返回空（邮件索引与摘要仍在，不丢信）。请在部署前就定好模式，不要中途改。
